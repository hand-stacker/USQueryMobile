  ...
  const loadTierAndUsage = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (hist?.error === "Authentication required." || hist?.error === "Invalid or expired token.") { return; }
        if (hist?.sessionId) {
          ...
          <View style={styles.inputBar}>
            ...
            >
              <Ionicons
                name="send"
                size={18}
                color={(!inputText.trim() || sending || chatDisabled) ? theme.subtext : "#fff"}
              />
            </Pressable>
          </View>
        </KeyboardAvoidingView>
...
function GateView({ isLoggedIn, navigation, theme, styles }: any) {
  return (
    <View style={styles.gateContainer}>
      ...
      {!isLoggedIn ? (
        <>
          <Pressable style={[styles.btnPrimary, styles.gateButtonTop]} onPress={() => navigation.navigate("Login")}>
            <Text style={styles.btnText}>Log in</Text>
          </Pressable>
          <Pressable style={[styles.btnGhost, styles.gateButtonSecondary]} onPress={() => navigation.navigate("Plans")}>
            <Text style={styles.btnGhostText}>See plans</Text>
          </Pressable>
        </>
      ) : (
        <Pressable style={[styles.btnPrimary, styles.gateButtonTop]} onPress={() => navigation.navigate("Plans")}>
  ...
  return (
    <View style={styles.emptyChatContainer}>
      ...
      <Text style={styles.emptyChatBody}>
        Ask anything about this bill: what it does, who it affects, or how it became law.
      </Text>
    </View>
  );
}
...
function UsageBar({ usage, tier, theme, styles }: any) {
      ...
      <View style={styles.usageBar}>
        ...
        <View style={styles.usageCountRow}>
          {Array.from({ length: total }).map((_, i) => (
            <View
              key={i}
              style={[styles.usageSegment, { backgroundColor: i < used ? theme.border : fill }]}
            />
          ))}
        </View>
      </View>
  ...
  if (usage.display === "donut") {
    ...
    return (
      <View style={styles.usageBar}>
        <View style={styles.usageDonutRow}>
...
function MessageBubble({ message, showSources, onToggleSources, theme, styles }: {
  ...
  return (
    <View style={[styles.bubbleWrapper, isUser ? styles.bubbleWrapperUser : styles.bubbleWrapperAI]}>
      <View style={[
        ...
        {!isUser && !message.isError ? (
          ...
          <Text style={[
            ...
            message.isError && styles.bubbleTextError,
          ]}>
            {message.text}
          </Text>
        )}
      ...
      {timeStr && (
        <Text style={[styles.timestampText, isUser && styles.timestampRight]}>{timeStr}</Text>
      )}

      {message.sources && message.sources.length > 0 && (
        <Pressable style={styles.sourcesToggle} onPress={onToggleSources}>
          <Ionicons
            name={showSources ? "chevron-up-outline" : "document-text-outline"}
            size={12}
            color={theme.primary}
          />
